...
const fetchFileData = async (authToken: string, file: { id: string; name: string; mimeType: string }) => {
...
interface ProcessorProps {
  processFunction: (authToken: string, folder: SelectedFolders) => Promise<any>;
  title: string;
  selectedFolder: SelectedFolders | null;
  disabled: boolean;
}
...
const Processor = ({
  ...
  const handleProcess = async () => {
    ...
    try {
      ...
      setStatus(`Error processing ${title.toLowerCase()}: ${error instanceof Error ? error.message : String(error)}`);
    } finally {
      setIsLoading(false);
    }
  };
  ...
  return (
    <div className="mb-8">
      ...
      <button
        ...
        className={`rounded-lg px-6 py-3 font-semibold text-white ${isLoading || disabled || !selectedFolder
      ...
      {status && (
        <div
          className={`mt-4 p-4 rounded-lg ${status.includes('Error')
...
const SheetsProcessor = () => {
  ...
  useEffect(() => {
    const loadFolders = async () => {
      ...
      try {
        ...
        setFolderError('Error loading folders: ' + (error instanceof Error ? error.message : String(error)));
      } finally {
  ...
  const processClientData = async (authToken: string, selectedFolders: SelectedFolders) => {
  ...
  const processStaffData = async (authToken: string, selectedFolders: SelectedFolders) => {
  ...
  const convertTo24Hr = (timeStr: string) => {
  ...
  const processAddressData = async (authToken: string, selectedFolders: SelectedFolders) => {
        ...
        <div className="bg-white rounded-lg shadow-lg p-6">
          ...
          <div className="space-y-8">
            <Processor
              ...
              selectedFolder={selectedFolders.clients ? { clients: selectedFolders.clients, therapists: '', supervisors: '' } : null}
              disabled={isLoadingFolders || !selectedFolders.clients}
            />
            <Processor
              title="Staff"
              processFunction={processStaffData}
              selectedFolder={selectedFolders.therapists || selectedFolders.supervisors ?
                { clients: '', therapists: selectedFolders.therapists, supervisors: selectedFolders.supervisors } : null}
            ...
            <Processor
              ...
              selectedFolder={selectedFolders.clients || selectedFolders.therapists || selectedFolders.supervisors ?
                selectedFolders : null}
              disabled={isLoadingFolders || (!selectedFolders.clients && !selectedFolders.therapists && !selectedFolders.supervisors)}
            />
